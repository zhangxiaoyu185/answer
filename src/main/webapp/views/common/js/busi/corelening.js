$(function () {
    ...
    var strhtml_searchContent = '<div class="inline-block margin">'
        + '<span>教辅名称:</span>'
        + '<input type="text" class="inputStyle_condition crlngName"/>'
        + '</div>';
    ...
    $('.addit').on('click', function () {
        layer.open({
            ...
            content: '../busi/corelening_add.html?timestamp=' + (new Date()).valueOf()
        });
    });
    //修改
    $("body").delegate('.modifyit', 'click', function () {
        layer.open({
            ...
            shadeClose: false, //点击遮罩关闭层
            area: [widthLayer, heightLayer],
            content: '../busi/corelening_modify.html?id=' + $(this).attr("data-id") + '&timestamp=' + (new Date()).valueOf()
        });
    });
    //全选
    ...
    $("body").delegate(".delthese", "click", function () {
        ...
        if (ids == "") {
            ...
            if (r == true) {
                var str = 'crlngUuids=' + encodeURIComponent(ids);
                getOData(str, "coreLening/delete/batch", {
                        fn: function (oData) {
                            var obj = {};//查询条件对象
                            searchContent(obj);
    ...
    $("body").delegate(".delit", "click", function () {
        ...
        if (r == true) {
            var str = 'crlngUuid=' + encodeURIComponent($(this).attr("data-id"));
            getOData(str, "coreLening/delete/one", {
                    fn: function (oData) {
                        var obj = {};//查询条件对象
                        searchContent(obj);
                        pagenum = parseInt($(".curpage").text());
...
function showList(obj, pagenum) {
    var aData = [{name: "<input type='checkbox' name='checkboxAll' value='checkbox' />", percent: "5"},
        {name: "教辅名称", percent: "70"},
        {name: "操作", percent: "25"}];
    setTableHead(aData);
    var str = 'pageNum=' + pagenum + '&pageSize=10';
    if (obj.crlngName != "") {
        str += '&crlngName=' + encodeURIComponent(obj.crlngName);
    }
    getOData(str, "coreLening/find/by/cnd", {
            fn: function (oData) {
                var strhtml_list = "";
                ...
                for (var i = 0; i < ln; i++) {
                    strhtml_list += '<tr class="trHighLight">'
                        + '<td>' + '<input type="checkbox" name="checkbox" value="checkbox" data-id="' + arrData[i].crlngUuid + '"/>' + '</td>'
                        + '<td>' + (arrData[i].crlngName || "") + '</td>'
                        + '<td>'
                        + '<a  class="p-edit modifyit" data-id="' + arrData[i].crlngUuid + '">修改</a>'
                        + '<a  class="p-edit delit" data-id="' + arrData[i].crlngUuid + '">删除</a>'
                        + '</td>'
                        + '</tr>';
                }
