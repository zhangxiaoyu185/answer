$(function () {
    ...
    var strhtml_searchContent = '<div class="inline-block margin">'
        + '<span>学校名称:</span>'
        + '<input type="text" class="inputStyle_condition crsclName"/>'
        + '</div>';
    ...
    $('.addit').on('click', function () {
        layer.open({
            ...
            content: '../busi/coreschool_add.html?timestamp=' + (new Date()).valueOf()
        });
    });
    //修改
    $("body").delegate('.modifyit', 'click', function () {
        layer.open({
            ...
            shadeClose: false, //点击遮罩关闭层
            area: [widthLayer, heightLayer],
            content: '../busi/coreschool_modify.html?id=' + $(this).attr("data-id") + '&timestamp=' + (new Date()).valueOf()
        });
    });
    //全选
    ...
    $("body").delegate(".delthese", "click", function () {
        ...
        if (ids == "") {
            ...
            if (r == true) {
                var str = 'crsclUuids=' + encodeURIComponent(ids);
                getOData(str, "coreSchool/delete/batch", {
                        fn: function (oData) {
                            var obj = {};//查询条件对象
                            searchContent(obj);
    ...
    $("body").delegate(".delit", "click", function () {
        ...
        if (r == true) {
            var str = 'crsclUuid=' + encodeURIComponent($(this).attr("data-id"));
            getOData(str, "coreSchool/delete/one", {
                    fn: function (oData) {
                        var obj = {};//查询条件对象
                        searchContent(obj);
                        pagenum = parseInt($(".curpage").text());
...
function showList(obj, pagenum) {
    var aData = [{name: "<input type='checkbox' name='checkboxAll' value='checkbox' />", percent: "5"},
        {name: "学校名称", percent: "70"},
        {name: "操作", percent: "25"}];
    setTableHead(aData);
    var str = 'pageNum=' + pagenum + '&pageSize=10';
    if (obj.crsclName != "") {
        str += '&crsclName=' + encodeURIComponent(obj.crsclName);
    }
    getOData(str, "coreSchool/find/by/cnd", {
            fn: function (oData) {
                var strhtml_list = "";
                ...
                for (var i = 0; i < ln; i++) {
                    strhtml_list += '<tr class="trHighLight">'
                        + '<td>' + '<input type="checkbox" name="checkbox" value="checkbox" data-id="' + arrData[i].crsclUuid + '"/>' + '</td>'
                        + '<td>' + (arrData[i].crsclName || "") + '</td>'
                        + '<td>'
                        + '<a  class="p-edit modifyit" data-id="' + arrData[i].crsclUuid + '">修改</a>'
                        + '<a  class="p-edit delit" data-id="' + arrData[i].crsclUuid + '">删除</a>'
                        + '</td>'
                        + '</tr>';
                }
