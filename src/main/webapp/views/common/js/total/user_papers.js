// JavaScript Document
$(function () {
	$(".cur_postion").html("当前位置： [考试统计] - [学生试卷]");
    ...
    $('.toExcel').on('click', function () {
        window.location = urlfile + "/gradeTotal/excel/export/userpapers?strLening="+$('.strLening').val();
    });
...
function showList(obj,pagenum){
	...
	getOData(str,"gradeTotal/user/papers",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				...
				+'<td><a class="p-edit seeDes" onclick="seeDes(\'未做试卷\',\''+arrData[i].hwzValue+'\')">'+(arrData[i].hwzKey || 0)+'</a></td>'
				+'<td><a class="p-edit seeDes" onclick="seeDes(\'已做未满分试卷\',\''+arrData[i].hyzwmfValue+'\')">'+(arrData[i].hyzwmfKey || 0)+'</a></td>'
				+'<td><a class="p-edit seeDes" onclick="seeDes(\'已做满分试卷\',\''+arrData[i].hyzmfValue+'\')">'+(arrData[i].hyzmfKey || 0)+'</a></td>'
                +'<td><a class="p-edit seeDes">'+(arrData[i].totalKey || 0)+'</a></td>'
				+'</tr>';
		}
